...
def test_dao_update_notifications_by_reference_set_returned_mail_status(sample_letter_template):
    ...
    updated_count = dao_update_notifications_by_reference(
        references=['ref'],
        update_dict={"status": "returned-mail"}
    )

    assert updated_count == 1
    assert Notification.query.get(notification.id).status == 'returned-mail'
